...
static sqlite3 *db;
static sqlite3_stmt *insert_block_stmt;  //sql语句
...
int db_init(char *path) {
    ...
    static const char *create_query =
        ...
        "create table if not exists state ("            //用来记录玩家状态
        "   x float not null,"                          //x,y,z三坐标
        "   y float not null,"
        "   z float not null,"
        "   rx float not null,"                         //x方向与y方向旋转角度
        "   ry float not null"
        ");"
        "create table if not exists block ("            //方块
        "    p int not null,"
        "    q int not null,"                           //p,q为区块
        "    x int not null,"                           //x,y,z为区块上的坐标
        "    y int not null,"
        "    z int not null,"
        "    w int not null"                            //w为方块种类
        ");"
    ...
    static const char *set_key_query =
        ...
        "values (?, ?, ?);";
    int rc;
    rc = sqlite3_open(path, &db);                                                   //打开数据库
    if (rc) return rc;
    rc = sqlite3_exec(db, create_query, NULL, NULL, NULL);                          //初始化
    if (rc) return rc;
    rc = sqlite3_prepare_v2(
        db, insert_block_query, -1, &insert_block_stmt, NULL);                      //准备插入方块的语句
    if (rc) return rc;
    rc = sqlite3_prepare_v2(
        db, insert_light_query, -1, &insert_light_stmt, NULL);                      //准备插入光线的语句
    if (rc) return rc;
    rc = sqlite3_prepare_v2(
    ...
    if (rc) return rc;
    rc = sqlite3_prepare_v2(db, load_blocks_query, -1, &load_blocks_stmt, NULL);    //准备加载方块的语句
    if (rc) return rc;
    rc = sqlite3_prepare_v2(db, load_lights_query, -1, &load_lights_stmt, NULL);    //准备加载光线的语句
    if (rc) return rc;
...
void db_close() {
    ...
    sqlite3_finalize(insert_block_stmt);                    //销毁前面被sqlite3_prepare创建的准备语句
...
void db_commit() {
    ...
    mtx_lock(&mtx);                                           //线程锁
    ring_put_commit(&ring);                                   //发送一个确认进队列
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
}
...
void db_save_state(float x, float y, float z, float rx, float ry) {         //保存玩家状态
    if (!db_enabled) {
        return;
    }
    ...
    sqlite3_stmt *stmt;
    sqlite3_exec(db, "delete from state;", NULL, NULL, NULL);               //删除原状态
    ...
    sqlite3_step(stmt);                                                     //保存现有状态
    sqlite3_finalize(stmt);
}

int db_load_state(float *x, float *y, float *z, float *rx, float *ry) {     //加载玩家状态
    if (!db_enabled) {
        return 0;
    }
    static const char *query =
...
void db_insert_block(int p, int q, int x, int y, int z, int w) {            
    ...
    mtx_lock(&mtx);
    ring_put_block(&ring, p, q, x, y, z, w);                                //发送一个方块插入请求进队列
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
}

void _db_insert_block(int p, int q, int x, int y, int z, int w) {           //插入方块
...
void db_insert_light(int p, int q, int x, int y, int z, int w) {
    ...
    mtx_lock(&mtx);
    ring_put_light(&ring, p, q, x, y, z, w);                                //发送一个光线插入请求进队列
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
}

void _db_insert_light(int p, int q, int x, int y, int z, int w) {           //插入灯光
...
void db_load_blocks(Map *map, int p, int q) {               //读取方块坐标和种类
    if (!db_enabled) {
        return;
    }
    mtx_lock(&load_mtx);
    ...
    while (sqlite3_step(load_blocks_stmt) == SQLITE_ROW) {
        ...
        map_set(map, x, y, z, w);                           //在地图上画出方块
    }
    mtx_unlock(&load_mtx);
}

void db_load_lights(Map *map, int p, int q) {               //读取光线坐标和种类
    if (!db_enabled) {
        return;
    }
    mtx_lock(&load_mtx);
...
void db_set_key(int p, int q, int key) {
    ...
    mtx_lock(&mtx);
    ring_put_key(&ring, p, q, key);                         //发送一个key进队列
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
}
...
void db_worker_start(char *path) {                  //开始线程
    if (!db_enabled) {
        return;
    }
    ring_alloc(&ring, 1024);                        //创建一个队列
...
void db_worker_stop() {                             //停止线程
    if (!db_enabled) {
        return;
    }
    mtx_lock(&mtx);
    ring_put_exit(&ring);                           //发送退出信号进队列
    cnd_signal(&cnd);
    mtx_unlock(&mtx);
    thrd_join(thrd, NULL);
    cnd_destroy(&cnd);
