...
void ring_alloc(Ring *ring, int capacity) {                             //分配一个queue buffer
...
void ring_free(Ring *ring) {                                            //释放
    free(ring->data);
}

int ring_empty(Ring *ring) {                                            //判断空
    return ring->start == ring->end;
}

int ring_full(Ring *ring) {                                             //判断满
    return ring->start == (ring->end + 1) % ring->capacity;
}

int ring_size(Ring *ring) {                                             //返回大小
...
void ring_grow(Ring *ring) {                                            //ring扩容
    Ring new_ring;
    RingEntry entry;
...
void ring_put(Ring *ring, RingEntry *entry) {                           //进队列(一项)
    if (ring_full(ring)) {
        ring_grow(ring);
    }
...
void ring_put_block(Ring *ring, int p, int q, int x, int y, int z, int w) { //进队列(块)
    RingEntry entry;
    entry.type = BLOCK;
    entry.p = p;
    entry.q = q;
...
void ring_put_light(Ring *ring, int p, int q, int x, int y, int z, int w) { //进队列(光)
    RingEntry entry;
    entry.type = LIGHT;
    entry.p = p;
    entry.q = q;
...
void ring_put_key(Ring *ring, int p, int q, int key) {  //进队列(key)
    RingEntry entry;
    entry.type = KEY;
    entry.p = p;
    entry.q = q;
...
void ring_put_commit(Ring *ring) {  //进队列(确认)
    RingEntry entry;
    entry.type = COMMIT;
    ring_put(ring, &entry);
}

void ring_put_exit(Ring *ring) {    //进队列(退出)
    RingEntry entry;
    entry.type = EXIT;
    ring_put(ring, &entry);
}

int ring_get(Ring *ring, RingEntry *entry) {    //得到队列第一项
    if (ring_empty(ring)) {
        return 0;
    }
